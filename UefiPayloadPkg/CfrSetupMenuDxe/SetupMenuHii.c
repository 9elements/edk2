  ...
  )
{
  ...
  CfrCreateRuntimeComponents (QuestionId);
  return EFI_SUCCESS;
}

/**
  Return the HiiStringId for the specified child node that has
  the provided tag.

  @param  [in]      Root          The Node currently being processed.
  @param  [in]      Tag           The tag of the child node to locate.
  @param  [out]     HiiStringId   The generated HiiStringId.
...
EFI_STATUS
EFIAPI
SetupMenuExtractHiiString (
  IN CONST UINT8          *Root,
  IN CONST UINT32         Tag,
  OUT      EFI_STRING_ID  *HiiStringId
  )
{
  EFI_STATUS  Status;
  CHAR16      *Str;

  if ((Root == NULL) || (HiiStringId == NULL)) {
    return EFI_INVALID_PARAMETER;
  }

  Status = CfrExtractString (Root, Tag, &Str);
  if (EFI_ERROR (Status)) {
    return Status;
  }

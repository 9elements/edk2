  ...
  )
{
  ...
  Response->Header.SourceEID   = MctpGetOwnEndpointID (Binding);
  Response->Header.FlagsSeqTag = Msg->Header.FlagsSeqTag;

  //
  // Clear TO flag
  //
  Response->Header.FlagsSeqTag &= ~(MCTP_TRANSPORT_HDR_TO_MASK << MCTP_TRANSPORT_HDR_TO_SHIFT);

  ControlHdr  = &Msg->Body.ControlMsg.Header;
  ResponseHdr = &Response->Body.ControlResponseMsg.Header;

  ResponseHdr->ICMsgType      = MCTP_TYPE_CONTROL_MSG;
  ResponseHdr->Rq             = 0;
  ResponseHdr->D              = 0;
  ResponseHdr->rsvd           = 0;
  ...
  )
{
  ...
  if (!MctpIsAssignableEndpointID (Req->EndpointID)) {
  ...
  } else if (MctpIsBusOwner (Binding)) {
    Status = EFI_INVALID_PARAMETER;
  } else {
    switch (Req->RequestData & 0x3) {
      ...
      case 1:
        ...
        //
        MctpSetOwnEndpointID (Binding, Req->EndpointID);
        MctpSetBusOwnerEndpointID (Binding, SourceEID);
        Status = EFI_SUCCESS;
        break;
      case 2:
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Resp.Status      = 0;
    Resp.EIDSetting  = MctpGetOwnEndpointID (Binding);
    Resp.EIDPoolSize = 0;
  ...
  )
{
  ...
  Resp.EndpointID   = MctpGetOwnEndpointID (Binding);
  Resp.EndpointType = 0;
  if (MctpIsBusOwner (Binding)) {
    Resp.EndpointType |= 0x10;
  }

  if (MctpSupportsStaticEID (Binding) && (MctpGetStaticEID (Binding) != MctpGetOwnEndpointID (Binding))) {
    Resp.EndpointType |= 0x3;
  } else if (MctpSupportsStaticEID (Binding) && (MctpGetStaticEID (Binding) == MctpGetOwnEndpointID (Binding))) {
    Resp.EndpointType |= 0x2;
  }

  Resp.MediumSpecific = MctpPhysicalGetEndpointInformation (Binding);
  ...
  )
{
  ...
  for (Index = 0; Index < Binding->NumSupportedMessageTypes; Index++) {
    Resp.Entry[Index] = Binding->SupportedMessageTypes[Index];
  }

  Resp.MessageTypeCount = Binding->NumSupportedMessageTypes;
  ...
  )
{
  ...
  if (Req->VendorIDSelector < Binding->NumSupportedVendorDefinedMessages) {
    Resp.VendorID = Binding->SupportedVendorMessage[Req->VendorIDSelector];
    if ((Req->VendorIDSelector + 1) < Binding->NumSupportedVendorDefinedMessages) {
  ...
  )
{
  ...
  if ((Hdr->ICMsgType & MCTP_MSG_TYPE_MASK) != MCTP_TYPE_CONTROL_MSG) {
    return EFI_UNSUPPORTED;
  }

  if (Hdr->ICMsgType & MCTP_MSG_TYPE_IC) {
    //
    // Not yet supported
    //
    return EFI_COMPROMISED_DATA;
  ...
  )
{
  ...
  switch (Hdr->CommandCode) {
    case MctpCmdSetEndpointID:
      ...
      Status = MctpResponseSetEndpointID (Binding, &Msg->Body.ControlMsg, Msg->Header.SourceEID, &Length, &ResponseMsg);
      break;
    case MctpCmdGetEndpointID:
      Status = MctpResponseGetEndpointID (Binding, &Length, &ResponseMsg);
      break;
    ...
    case MctpCmdGetMCTPMessageTypeSupport:
      Status = MctpResponseGetMCTPMessageTypeSupport (Binding, &Length, &ResponseMsg);
      break;
    case MctpCmdGetMCTPVendorMessageSupport:
      ...
      Status = MctpResponseVendorDefinedMessageType (Binding, &Msg->Body.ControlMsg, &Length, &ResponseMsg);
      break;
  ...
  MctpDataDescriptor.Length    = Length;
  MctpDataDescriptor.Size      = Length;
  MctpDataDescriptor.DataBlock = &ResponseMsg.Body.Raw;
